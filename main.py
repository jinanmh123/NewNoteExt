...
def rm_unknowns(avgs1, avgs2):
    ...
    for i,note in enumerate(notes2):
        if note not in notes1:
            notesr.append(notes2[i])

    return list(set(notesr))
...
def nd(original_notes, user_notes):   
    ...
    if cust_round_for_cons((abs(len(user_notes)-len(A))/len(user_notes))*10) < 8:
        return cust_round_for_cons((abs(len(user_notes)-len(A))/len(user_notes))*10)
    else:
        rs = comp(original_notes, user_notes, 5, 0.5)
        return rs
...
def main_concurrently(original_notes, user_notes):
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = {
            executor.submit(pa, original_notes, user_notes): "Pitch Accuracy",
            executor.submit(tr, original_notes, user_notes): "Timing/Rhythm",
            executor.submit(nd, original_notes, user_notes): "Note Duration",
            executor.submit(tc, original_notes, user_notes): "Tempo Consistency",
            executor.submit(d, original_notes, user_notes): "Dynamics",
            executor.submit(a, original_notes, user_notes): "Articulation",
            executor.submit(c, original_notes, user_notes): "Consistency",
        }
        
        results = {futures[future]: future.result() for future in concurrent.futures.as_completed(futures)}
        dats = ["Pitch Accuracy", "Timing/Rhythm", "Note Duration", "Tempo Consistency", "Dynamics", "Articulation", "Consistency"]
        result = "<br>".join([f"{dat}: {results[dat]}/10" for dat in dats])
    return result
...
def analyze():
    if 'original_file' not in request.files and 'user_played_file' not in request.files:
        return 'Missing files', 400

    original_file = request.files['original_file']
    user_file = request.files['user_played_file']

    orig_file_path = "uploads/original.wav"
    user_file_path = "uploads/user.wav"
